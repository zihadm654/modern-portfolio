...
The `config` folder houses various files ready for customization to suit your
needs.
...
Within `landing.ts`, discover data configurations for different sections on the
homepage.
...
`marketing.ts`, `dashboard.ts` and `docs.ts` contain configuration links
tailored for their respective sections.
